	...
	public static ArrayList<Post> run(String url) throws MalformedURLException, JsonMappingException, JsonProcessingException, JarException, UrlNotFoundException, JarException, JSONException, IOException  {
		...
		for(int i=0; i<t.length();i++){
			...
			for(int j=0; j<arrpost.size(); j++) {
				...
				String str3 = null;
			try {
				str3 = Read.readUrl("https://graph.facebook.com/" +arrpost.get(j).getId() +"?fields=attachments{media_type}&access_token=EAACae8EWj6UBAPCnGksMwLUdD6QnZCPiWMvaHTURou7rwJ2VDc1VpgMCWlFPuO8THGhR9pcQQebvZCrC9ZB9RzwKR8yTiuNDoLipnQoT1qh5hcmbRmdkPZBrfNlufrUvR7ZBXhl0aWRHuDxOVzGE488PZAokc6CfzKmRyzTOthZBi99f6xgY2CD9bjk8MAmVCkZD");
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
